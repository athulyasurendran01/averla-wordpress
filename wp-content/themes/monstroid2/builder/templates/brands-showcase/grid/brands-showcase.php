...
$layout = array(
	'sm' => $this->_var( 'columns_phone' ),
	'md' => $this->_var( 'columns_tablet' ),
	'lg' => $this->_var( 'columns_laptop' ),
	'xl' => $this->_var( 'columns' ),
);

$_layout_classes = array();

foreach ( $layout as $device => $col ) {
	$_layout_classes[] = sprintf( 'columns-%2$s-%1$s', intval( $col ), $device );
}

$layout_classes = join( ' ', $_layout_classes );
?>
<?php if ( ! empty( $children ) ) : ?>
	...
	<div<?php echo $wrapper_atts; ?>>
		...
		<div class="row tm_pb_brands_showcase_module__brands <?php echo $layout_classes; ?>">
